 ...
 * @internal
 */
final class GetBlockGroupByNameHandler extends AbstractBlockGroupQueryHandler
{
    /**
     * @throws \PrestaShopException
     * @throws BlockGroupNotFoundException
     */
    public function handle(GetBlockGroupByName $query): ?BlockGroup
    {
        try {
            $blockGroup = $this->entityRepository->findByName($query->getName()->getValue());
        } catch (\PrestaShopException $e) {
            $message = sprintf(
                'An unexpected error occurred when retrieving block group with name %s',
                var_export($query->getName()->getValue(), true)
            );
